function [X,Pa] = AMP_cellfree(Y,S,gamma_w,lsfc,AMP_option)
    ...
    for k=1:K
        for m=1:M
            ...
            for i=1:length(aclist)
                ...
                Mu(n,m,k)    = V(n,m,k) * Gamma(m,k)/(V(n,m,k) * Gamma(m,k) ...
                    + 1) * R(n,m,k); % Mean of channel
                Sigma(n,m,k) = real(V(n,m,k)/(V(n,m,k) * Gamma(m,k) + 1)); % Variance of channel
                X_hat(n,m,k) = Damp * X_pre(n,m,k) + (1-Damp) * Pi(n,m,k) ...
                    * Mu(n,m,k); % Posterior mean
                X_var(n,m,k) = Pi(n,m,k) * Sigma(n,m,k); % Posterior variance
                phi_temp     = 1/Pi(n,m,k);
                omega_temp   = 1 + (Gamma(m,k)^2 * V(n,m,k) * (phi_temp-1) ...
                    * norm(R(n,m,k),2)^2)/((Gamma(m,k)*V(n,m,k) + 1) * phi_temp);
                alpha_temp   = (Gamma(m,k)*V(n,m,k))/(Gamma(m,k)*V(n,m,k)+1) ...
                    * (omega_temp/phi_temp);
            ...
            if strcmp(AMP_option, 'vector AMP') == 1
                ...
                d = gamma_w * (gamma_w * Bar_S .* Bar_S + gamma_tilde ...
                    * eye(Rank_S))^(-1) * diag(Bar_S .* Bar_S);
                Gamma(m,k) = Damp * Gamma_pre(m,k) + (1-Damp) ...
                    * real(gamma_tilde * mean(d) / (N/Rank_S - mean(d))); % Update Gamma(m,k)
                R(:,m,k) = R_tilde + (N/Rank_S) * Bar_V * diag(d/mean(d)) ...
                    * (Y_tilde(:,m,k) - Bar_V' * R_tilde);
            elseif strcmp(AMP_option, 'AMP') == 1
                Vk(:,m,k)  = Y(:,m,k) - S*X_hat(:,m,k) + (N/L)*alpha_m*Vk(:,m,k);
                R(:,m,k)   = X_hat(:,m,k) + S'*Vk(:,m,k);
                Gamma_temp = real(1/(1/gamma_w+(N/L)*(mean(X_hat(:,m,k) ...
                    .*conj(X_hat(:,m,k))+X_var(:,m,k)))));
                Gamma(m,k) = Damp * Gamma_pre(m) + (1-Damp) * Gamma_temp;
            end
        end
    end
